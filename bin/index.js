  ...
  (initErr, initStdout, initStderr) => {
    ...
    for (let i = 0; i < filesToCopy.length; i += 1) {
      fs.createReadStream(path.join(__dirname, `../${filesToCopy[i]}`)).pipe(
        fs.createWriteStream(`${packageJson.name}/${filesToCopy[i]}`)
      );
    }

    fs.createReadStream(path.join(__dirname, `../.gitignorelocal`)).pipe(
      fs.createWriteStream(`${packageJson.name}/.gitignore`)
    );
